...
def lookup(driver):
    driver.get('https://www.m13.com/product/postcard-printing')
    ...
    for option in selectPrintSize.options:
        ...
        if selectPrintSizeValue != '' and selectPrintSizeValue != '637':
    ...
    for x in range(len(optionValuesCombination)):
        ...
        try:
            rows.append([
                ...
                service_prices[3][1], 
                service_days[3],
                'Exact Quantity=0, PRINT SIZE={}, EXACT SIZE=0, SIDES={}, PAPER OR STOCK={}, COATING={}'.format(selectPrintSizeText, selectSideText, selectPaperStockText, selectCoatingText), 
                selectQuantityText
            ])
...
def excel_file(rows):
    ...
    with open('/home/apptrinity19/development/scraping/web_scraper/files/postcard-printing_' + str(date.today()) +  '.csv', 'w', newline='') as f_output:
