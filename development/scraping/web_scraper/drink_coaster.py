    ...
    for x in range(5):
        ...
        try:
            rows.append([product_id, service_prices[0][0], service_prices[0][1], service_days[0], service_prices[1][0], service_prices[1][1], service_days[1], service_prices[2][0], service_prices[2][1], service_days[2], service_prices[3][0], service_prices[3][1], service_days[3],
                         'PRINT SIZE={}; SIDES={}; PAPER OR STOCK={}, COLOR CRITICAL=0, Stock Diecut=0'.format(selectPrintSizeText, selectSideText, selectPaperStockText), selectQuantityText
                         ])
...
def excel_file(rows):
    ...
    # with open(os.path.join(os.getcwd(), 'files/drink-coaster_' + str(date.today()) +  '.csv'), 'w', newline='') as f_output:
    with open('/home/apptrinity19/development/scraping/web_scraper/files/drink-coaster111_' + str(date.today()) +  '.csv', 'w', newline='') as f_output:
