...
def lookup(driver):
    driver.get('https://www.m13.com/product/stickers-printing')
    ...
    for option in selectPrintSize.options:
        ...
        if selectPrintSizeValue != '' and selectPrintSizeValue != '643':
    ...
    for x in range(len(optionValuesCombination)):
        ...
        if str(optionValuesCombination[x][3]) == '118':
            selectCoating.select_by_value("3")
        elif str(optionValuesCombination[x][3]) =='115':
            selectCoating.select_by_value(str(optionValuesCombination[x][4]))
        else:
            selectCoating.select_by_value("")
        ...
        try:
            rows.append([
                ...
                service_prices[3], 
                service_days[3],
                'PRINT SIZE={}, SIDES={}, PAPER OR STOCK={}, COATING={}, COLOR CRITICAL=0, Corner Rounding=0'.format(selectPrintSizeText, selectSideText, selectPaperStockText, selectCoatingText), 
                selectQuantityText
            ])
...
def excel_file(rows):
    ...
    with open('/home/apptrinity19/development/scraping/web_scraper/files/stickers-printing_' + str(date.today()) +  '.csv', 'w', newline='') as f_output:
