...
def lookup(driver):
    driver.get('https://www.m13.com/product/booklet-printing')
    ...
    selectCoverPaper = Select(driver.find_element_by_id("Order_CoverStockId"))
    ...
    selectCoverPaperValues = []
    for option in selectCoverPaper.options:
        selectCoverPaperValue = option.get_attribute("value")
        if selectCoverPaperValue != '':
            selectCoverPaperValues.append(selectCoverPaperValue)
    # print(selectCoverPaperValues)
    ...
    for x in range(len(optionValuesCombination)):
        ...
        if str(optionValuesCombination[x][1]) != '622':
        ...
        selectCoverPaper.select_by_value(str(optionValuesCombination[x][5]))
        selectPage.select_by_value(str(optionValuesCombination[x][6]))
        selectBinding.select_by_value(str(optionValuesCombination[x][7]))
        # print(x)
        ...
        if optionValuesCombination[x][5] in selectCoverPaperValues:
            selectCoverPaperText = selectCoverPaper.first_selected_option.text

        if optionValuesCombination[x][6] in selectPageValues:
            selectPageText = selectPage.first_selected_option.text

        if optionValuesCombination[x][7] in selectBindingValues:
        ...
        try:
            rows.append([
                ...
                service_prices[3][1], 
                service_days[3],
                'Exact Quantity=0, PRINT SIZE={}, EXACT SIZE=0, SIDES={}, PAPER OR STOCK={}, COATING={}, PAGES={}, COLOR CRITICAL=0, COVER PAPER TYPE={}, BINDING={}'.format(selectPrintSizeText, selectSideText, selectPaperStockText, selectCoatingText, selectPageText, selectCoverPaperText, selectBindingText), 
                selectQuantityText
            ])
...
def excel_file(rows):
    ...
    with open('/home/apptrinity19/development/scraping/web_scraper/files/booklet-printing_' + str(date.today()) +  '.csv', 'w', newline='') as f_output:
