...
def lookup(driver):
    driver.get('https://www.m13.com/product/brochure-printing')
    ...
    selectFolding = Select(driver.find_element_by_id("SelectedOrderOptions_13__Value"))
    ...
    for option in selectPrintSize.options:
        ...
        if selectPrintSizeValue != '' and selectPrintSizeValue != '624':
    ...
    for x in range(len(optionValuesCombination)):
        ...
        if str(optionValuesCombination[x][3]) == '3' and str(optionValuesCombination[x][3]) == '5':
        ...
        try:
            rows.append([
                ...
                service_prices[3][1], 
                service_days[3],
                'PRINT SIZE={}, SIDES={}, PAPER OR STOCK={}, COATING={}, COLOR CRITICAL=0, FOLDING={},SCORE=0, Wafer Seal=0'.format(selectPrintSizeText, selectSideText, selectPaperStockText, selectCoatingText, selectFoldingText), 
                selectQuantityText
            ])
...
def excel_file(rows):
    ...
    with open('/home/apptrinity19/development/scraping/web_scraper/files/brochure_printing_' + str(date.today()) +  '.csv', 'w', newline='') as f_output:
