...
def lookup(driver):
    driver.get('https://www.m13.com/product/newsletter-printing')
    ...
    selectFolding = Select(driver.find_element_by_id("SelectedOrderOptions_9__Value"))
    ...
    for option in selectPrintSize.options:
        ...
        if selectPrintSizeValue != '' and selectPrintSizeValue != '651':
    ...
    for option in selectFolding.options:
        ...
        if selectFoldingValue != '' and selectFoldingValue != '0' and selectFoldingValue != '12' and selectFoldingValue != '13':
    ...
    for x in range(len(optionValuesCombination)):
        ...
        try:
            rows.append([
                ...
                service_prices[3][1], 
                service_days[3],
                'Exact Quantity=0, PRINT SIZE={}, SIDES={}, PAPER OR STOCK={}, COATING={}, COLOR CRITICAL=0, Spot UV Gloss Coating=0, FOLDING={}, Variable Data=0'.format(selectPrintSizeText, selectSideText, selectPaperStockText, selectCoatingText, selectFoldingText), 
                selectQuantityText
            ])
...
def excel_file(rows):
    ...
    with open('/home/apptrinity19/development/scraping/web_scraper/files/newsletter-printing_' + str(date.today()) +  '.csv', 'w', newline='') as f_output:
